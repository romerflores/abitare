...
function RegistroConsumo() {
    ...
    useEffect(() => {
        const obtenerDepartamentos = async () => {
            try {
                const conexion = await fetch('https://abitare-back-production.up.railway.app/api/consumo/servicios');
                const respuesta = await conexion.json();
                if (!conexion.ok) throw new Error(respuesta.message);
    ...
    }, [formData.medidaRegistro, formData.servicio, formData.departamento]);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const conexion = await fetch('https://abitare-back-production.up.railway.app/api/consumo/registrar-consumo', {
                method: 'POST',
                headers: {
