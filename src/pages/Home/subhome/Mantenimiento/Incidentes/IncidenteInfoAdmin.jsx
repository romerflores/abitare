...
const IncidenteInfoAdmin = () => {
    ...
    useEffect(() => {
        const obtenerIncidente = async () => {
            try {
                const conexion = await fetch(`http://localhost:10000/api/administrador/obtener-incidente/:${idIncidente}`);
                if (!conexion.ok) {
    ...
    useEffect(() => {
        if (infoIncidente && infoIncidente.estado_out == 'PENDIENTE') {
            const obtenerPersonal = async () => {
                try {
                    const conexion = await fetch(`http://localhost:10000/api/administrador/personal-por-tipo/:${infoIncidente.mantenimiento_out}`);
                    const respuesta = await conexion.json();
    ...
    const asignarPersonal = async (e) => {
        ...
        try {
            const conexion = await fetch('http://localhost:10000/api/administrador/asignar-personal-incidente', {
                method: 'POST',
                headers: {
